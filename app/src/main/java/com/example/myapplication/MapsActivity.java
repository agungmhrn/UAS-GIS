...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void getlokasi (){
        String url="https://pawsgis.000webhostapp.com/paws.php";
                        ...
                        for (int i = 0; i < jumData; i++) {
                            try {
                                ...
                                mMap.addMarker(new MarkerOptions()
                                        .position(latlng[i])
                                        .snippet(alamat[i])
                                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.pawmark))
                                        .title(nama[i]));
                                ...
                                for(int i=0; i<jumData;i++){
                                    if(marker.getTitle().equals(nama[i])){
                                        if(MarkerD[i]){
                                            ...
                                            Toast pesan = Toast.makeText(MapsActivity.this, "Silahkan Klik Sekali Lagi", Toast.LENGTH_LONG);
